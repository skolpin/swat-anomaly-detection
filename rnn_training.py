...
class LSTM_model:
    ...
    def __init__(self, train_data, neurons, opt, loss_func, model = Sequential(), batch_size = 1,
                 train_loss = [np.nan], val_loss = [np.nan], epochs = [0], train_time = 0):
        ...
        self.model.add(LSTM(neurons, batch_input_shape=(batch_size, X.shape[1], X.shape[2]), stateful=True, return_sequences=False)) #!!!
        #model.add(LSTM(5, stateful=True))
    ...
    def train(self, train_data, val_data, nb_epoch):
        X, y = train_data[:, 0:-1], train_data[:, -1]
        X = X.reshape(X.shape[0], 1, X.shape[1])
        X_val, y_val = val_data[:, 0:-1], val_data[:, -1]
        X_val = X_val.reshape(X_val.shape[0], 1, X_val.shape[1])
        fig, ax = plt.subplots()
        last_epoch = self.epochs[-1]
        t1 = time.time()
        for i in range(nb_epoch):
            H = self.model.fit(X, y, epochs=1, batch_size=self.batch_size, verbose=0, shuffle=False, validation_data=(X_val, y_val))
            self.train_loss += H.history['loss']
            self.val_loss += H.history['val_loss']
            self.epochs += [last_epoch+i+1]
            self.model.reset_states()
            ax.cla()
            ax.plot(self.epochs, self.train_loss)
            ax.plot(self.epochs, self.val_loss)
            ax.set_ylim(bottom = 0)
            ax.set_xlabel('epochs')
            ax.set_ylabel('loss')
            ax.set_title('Model loss by epochs')
        ...
        h,m,s = sec2time(self.train_time)
        print('The training was completed in {:d}h {:d}m {:d}s ({:.2f} sec/epoch).'.format(h,m,s,self.train_time/self.epochs[-1]))
        
    def forecast(self, X, batch_size=1):
        X = X.reshape(1, 1, len(X))
